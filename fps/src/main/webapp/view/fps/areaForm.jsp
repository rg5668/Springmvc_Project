...
<body class="w3-light-grey">
  ...
  <div class="w3-container w3-margin-left w3-margin-top">
    ...
    <c:choose>
		<c:when test="${area_code eq '20'}">
			<p style="font-size: 20px"><b><c:out value="${'서울'}" /> 여행지</b>
		</c:when>

		<c:when test="${area_code eq '32'}">
			<p style="font-size: 20px"><b><c:out value="${'인천'}" /> 여행지</b>
		</c:when>
		
		<c:when test="${area_code eq '31'}">
			<p style="font-size: 20px"><b><c:out value="${'경기'}" /> 여행지</b>
		</c:when>
		
		<c:when test="${area_code eq '33'}">
			<p style="font-size: 20px"><b><c:out value="${'강원도'}" /> 여행지</b>
		</c:when>
		
		<c:when test="${area_code eq '42'}">
			<p style="font-size: 20px"><b><c:out value="${'대전'}" /> 여행지</b>
		</c:when>
		
		<c:when test="${area_code eq '53'}">
			<p style="font-size: 20px"><b><c:out value="${'대구'}" /> 여행지</b>
		</c:when>
		
		<c:when test="${area_code eq '51'}">
			<p style="font-size: 20px"><b><c:out value="${'부산'}" /> 여행지</b>
		</c:when>
		
		<c:when test="${area_code eq '62'}">
			<p style="font-size: 20px"><b><c:out value="${'광주'}" /> 여행지</b>
		</c:when>
		
		<c:when test="${area_code eq '64'}">
			<p style="font-size: 20px"><b><c:out value="${'제주'}" /> 여행지</b>
		</c:when>
	</c:choose>
    ...
    <a href="<%=request.getContextPath() %>/allForm" style="font-size: 12px"> 전체보기 /</a>
      	<a href="<%=request.getContextPath() %>/areaForm?area_code=20" style="font-size: 12px"> 서울 /</a>
    	<a href="<%=request.getContextPath() %>/areaForm?area_code=32" style="font-size: 12px"> 인천 /</a>
    	<a href="<%=request.getContextPath() %>/areaForm?area_code=31" style="font-size: 12px"> 경기 /</a>
    	<a href="<%=request.getContextPath() %>/areaForm?area_code=33" style="font-size: 12px"> 강원도 /</a>
    	<a href="<%=request.getContextPath() %>/areaForm?area_code=42" style="font-size: 12px"> 대전 /</a>
    	<a href="<%=request.getContextPath() %>/areaForm?area_code=53" style="font-size: 12px"> 대구 /</a>
    	<a href="<%=request.getContextPath() %>/areaForm?area_code=51" style="font-size: 12px"> 부산 /</a>
    	<a href="<%=request.getContextPath() %>/areaForm?area_code=62" style="font-size: 12px"> 광주 /</a>
    	<a href="<%=request.getContextPath() %>/areaForm?area_code=64" style="font-size: 12px"> 제주</a>
  </div>
